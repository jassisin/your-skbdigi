...
$dp=$row['actual_dp'];
  ...
  $rcp=$row['rcp'];
  $qty=$row['qty'];
  $withoutgst=$row['withoutgst'];
  $mrp=$row['mrp'];
  $amount=$row['amount'];

  $total_billamount=$row['total_billamount'];
...
if (isset($_POST['submit'])) {
    ...
    foreach ($barcode as $index => $s_barcode) {
        ...
        if ($s_barcode != '' || $s_barcode != 'NULL') {
            ...
            //     $sql = "INSERT INTO entry(aid, vendor, store, mode, invno, indate, date_invoice, barcode, product_name, expiry_date, hsncode, gst, landing, qty, withoutgst, mrp, amount, total_billamount, totalamount, rcp, actual_dp, pm_margin, store_margin, pm_profit, store_profit, remark) VALUES ('$s_tid', '$s_vendor', '$s_store', '$s_mode', '$s_invno', '$s_date', '$invdate', '$s_barcode', '$s_pr_name', '$s_expiry', '$s_hsncode', '$s_gst', '$s_landing', '$s_qty', '$s_withoutgst', '$s_mrp', '$s_total', '$s_billamt', '$totalamount', '$s_rcp', '$s_dp', '$pm_margin', '$store_margin', '$pm_profit', '$store_profit', '$remark')";
        ...
        $data = array(
            ...
            'expiry_date' => $s_expiry,
            'hsncode' => $s_hsncode,
            'gst' => $s_gst,

            'qty' => $s_qty,
            'withoutgst' => $s_withoutgst,
            'mrp' => $s_mrp,
            'amount' => $s_total,
            'total_billamount' => $s_billamt,
        ...
        edit_row7('entry', $data, $conn, $s_tid);

        // $sql = "INSERT INTO entry(aid, vendor, store, mode, invno, indate, date_invoice, barcode, product_name, 
        // expiry_date, hsncode, gst, landing, qty, withoutgst, mrp, amount, total_billamount, totalamount, rcp,
        //  actual_dp, pm_margin, store_margin, pm_profit, store_profit, remark) VALUES ('$s_tid', '$s_vendor', '$s_store',
        //   '$s_mode', '$s_invno', '$s_date', '$invdate', '$s_barcode', '$s_pr_name', '$s_expiry', '$s_hsncode', '$s_gst',
        //    '$s_landing', '$s_qty', '$s_withoutgst', '$s_mrp', '$s_total', '$s_billamt', '$totalamount', '$s_rcp', '$s_dp', 
        //    '$pm_margin', '$store_margin', '$pm_profit', '$store_profit', '$remark')";
        // $res = mysqli_query($conn, $sql);
    }
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-6">
                        <div class="input-group mb-3">
                            ...
                            <select name="vendor" class="form-control itemname" data-live-search="true"  onchange="Disval(this);" placeholder="--Select vendor--" value="<?=$vendor?>">
                    ...
                    <div class="input-group mb-3">
                            ...
                            <input type="text" class="form-control invno" placeholder=""  name="invno" value="<?=$invno?>" >
                        </div>
                       
                        <div class="input-group mb-3">
                            <span class="input-group-text" >Date</span>
                            <input type="date" class="form-control"  name="date" required value="<?=$indate?>">
                        </div>
                        <div class="input-group mb-3">
                            <span class="input-group-text" >Inv Date</span>
                            <input type="date" class="form-control"  name="invdate" required value="<?=$date_invoice?>">
                        </div>
              ...
              <div style="overflow-x:scroll;">
                <table class="table table-bordered">
                    ...
                    <tbody id="TBody">
                    <?php
                        $sql2="select * from entry where invno='$invno' and MODE='IN' and date_invoice='$date_invoice' and active=0";
                      ...
                      <tr id="TRow" >
                        ...
                        <td ><input style="width:100px;" type="text" class="form-control text-end barcode" name="barcode[]" onchange="Master(this);" value="<?=$row['barcode']?>"></td>
                        <input type="hidden" class="form-control text-end tid" name="tid[]" value="<?=$row['id']?>">
                        <td ><input style="width:100px;" type="text" class="form-control text-end pr_name" name="pr_name[]" value="<?=$row['product_name']?>"></td>
                       


                        <td ><input  type="date" class="form-control expiry"  name="expiry[]" required value="<?=$expiry_date?>"></td>
                        <td ><input style="width:100px;" type="text" class="form-control text-end hsncode" name="hsncode[]" value="<?=$row['hsncode']?>"></td>
                        <td ><input type="text" style="width:100px;" class="form-control text-end dp" name="dp[]" value="<?=$row['actual_dp']?>" ></td>

                        <td ><input style="width:100px;" type="text" class="form-control text-end mrp" name="mrp[]" value="<?=$row['mrp']?>" ></td>

                        <td ><input style="width:100px;" type="text" class="form-control text-end landing" name="landing[]" value="<?=$row['landing']?>" oninput="CalcR(this);"></td>
                        <td ><input style="width:100px;" type="text" class="form-control text-end rcp" name="rcp[]" value="<?=$row['rcp']?>"></td>
                        <td ><input style="width:100px;" type="text" class="form-control text-end gst" name="gst[]" value="<?=$row['gst']?>"></td>
                       
                      
                        <td ><input style="width:100px;" type="text" class="form-control text-end withoutgst" name="withoutgst[]" value="<?=$row['withoutgst']?>" ></td>
                        <td ><input style="width:100px;" type="text" class="form-control text-end qty" name="qty[]"  oninput="CalcT(this);" value="<?=$row['qty']?>"></td>
                       
                        <td ><input style="width:100px;" type="text" class="form-control text-end total" name="total[]" value="<?=$row['amount']?>" readonly></td>
                  ...
                  <div class="row">
                    ...
                    <div class="col-6">
                        <div class="input-group mb-3">
                            ...
                            <input type="number" class="form-control text-end" id="FTotal" name="FTotal" value="<?=$total_billamount?>" disabled="">
                        </div>
                        <div class="input-group mb-3">
                            <span class="input-group-text" >Bill Amt</span>
                            <input type="number" class="form-control text-end" id="FGST" name="FGST" value="<?=$totalamount?>" required>
                        </div>
                        <!-- <div class="input-group mb-3">
