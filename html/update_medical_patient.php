...
if ($pid) {
    // Always update medical_table
    $stmt = $conn->prepare("UPDATE medical_table SET status=?, notes=?, next_visit_date=? WHERE PID=?");
    $stmt->bind_param("ssss", $status, $notes, $next_visit_date, $pid);
    $medical_updated = $stmt->execute();
    $stmt->close();
    ...
    if ($medical_updated && $reception_updated) {
