...
if(isset($_GET['did']) && ($_GET['did']!='')) {
  $did=$_GET['did'];
  $sql2="UPDATE task SET active=1 WHERE id='$did'";
...
$query = "SELECT column_name, is_visible FROM task_visibility";
...
if($username=='admin1'){
  $query = "SELECT $columnsToSelect FROM task WHERE active=0 ";
}
else{
$query = "SELECT $columnsToSelect FROM task WHERE active=0 and addedby='$username'";
}
$stmt = mysqli_query($conn, $query);
          ...
          <div class="card">
            ...
            <div class="table-responsive text-nowrap">
              <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post">					
                <button type="submit" id="export_data" name='export_data1' value="Export to excel" class="btn btn-primary btn-lg">Download Task List</button>
              </form>
              <table class="table">
                <thead>
                  ...
                  <?php foreach ($data as $row): ?>
                    <tr>
                      ...
                      <?php if (isset($row['id'])): ?>
                        ...
                        <td><a href="task-list.php?did=<?=$row['id']?>">Delete</a></td>
                      <?php else: ?>
                        <td>Invalid ID</td>
                      <?php endif; ?>
                    </tr>
