...
{
  $id=$_GET['did'];
  $sql2="update stocktab set active=1 where id='$id'";
  $res2=mysqli_query($conn,$sql2);
  header("Location: stock-check.php");
 echo '<script> window.location.href = "stock-check.php"; </script>';
}
if(isset($_POST['submit'])) {
  $store = $_POST['store'];
  $vendor = $_POST['vendor'];
  echo '<script>window.open("purchase-order.php?s=' . $store . '&v=' . $vendor . '", "_blank");</script>';
