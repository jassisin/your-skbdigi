...
$edit=1;
$eid=$_GET["eid"];
$sql2="select * from addressbook where id='$eid'";
...
if($edit!=1){
    ...
    $stmt = $conn->prepare("INSERT INTO addressbook (customername, status, leadsource, contactperson, email, phone, address, indate, action, addedby) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssssss", $cusname, $status, $leadsource, $contactper, $email, $phone, $address, $date, $action, $username);

    if ($stmt->execute()) {
        // Query executed successfully, redirect to address-book.php
  ...
  elseif($edit==1){
    ...
    $leadsourcee = $_POST['leadsource'];
    $contactpere = $_POST['contactperson'];
    $emaile = $_POST['email'];
    $phonee = $_POST['phone'];
    $addresse = $_POST['address'];
    $actione = $_POST['action'];
    $timezone = new DateTimeZone("Asia/Kolkata");
    $date = new DateTime();
    $date->setTimezone($timezone);
    $date = $date->format('Y-m-d');
    $data = array(
      'customername' => $cusnamee,
      'status' => $statuse,
      'leadsource' => $leadsourcee,
      'contactperson' => $contactpere,
      'email' => $emaile,
      'phone' => $phonee,
      'address' => $addresse,
      'indate' => $date,
      ...
      edit_row2('addressbook',$data,$conn,$eid);
      header("Location: address-book.php");
      echo '<script> window.location.href = "address-book.php"; </script>';
...
if(isset($_POST['submit2'])){
    ...
    if($extension == 'xlsx' || $extension == 'xls' || $extension == 'csv'){
        ...
        foreach($data as $row){
            ...
            // Use prepared statements to prevent SQL injection
            $stmt = $conn->prepare("INSERT INTO addressbook (customername, status, leadsource, contactperson, email, phone, address, indate, action, addedby) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssssssss", $name, $status, $lead_source, $contact_per, $email, $phone, $address, $in_date, $action, $username);
                  ...
                  <div class="card mb-4">
                    ...
                    <div class="card-body">
                      <form method="post">
                        ...
                        <div class="form-floating form-floating-outline mb-4">
                          <input type="text" class="form-control" id="basic-default-company" name="leadsource" placeholder="ACME Inc." value="<?=$leadsource?>"/>
                          <label for="basic-default-company">Lead Source</label>
                        </div>
                        <div class="form-floating form-floating-outline mb-4">
                          <input type="text" class="form-control" id="basic-default-company" name="contactperson" placeholder="ACME Inc." value="<?=$contactperson?>"/>
                          ...
                          <div class="input-group input-group-merge">
                            <div class="form-floating form-floating-outline">
                              <input
                                ...
                                aria-label="john.doe"
                                aria-describedby="basic-default-email2" name="email" value="<?=$email?>"/>
                        ...
                        <div class="form-floating form-floating-outline mb-4">
                          <input
                            ...
                            placeholder="658 799 8941" name="phone" value="<?=$phone?>"/>
                        ...
                        <div class="form-floating form-floating-outline mb-4">
                          <input type="textarea"
                            ...
                            style="height: 60px" name="address" value="<?=$address?>"></textarea>
                         ...
                         <div class="input-group mb-3">
                            ...
                            <?php
                                ...
                                echo"<select id='searchddl' name='action' class='form-select' value='$action' >";
                                 while($row=mysqli_fetch_array($result))
                                {
