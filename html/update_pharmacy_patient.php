...
if ($pid) {
    ...
    $stmt = $conn->prepare("UPDATE pharmacy_table SET status=?, notes=?, next_visit_date=? WHERE PID=?");
    $stmt->bind_param("ssss", $status, $notes, $next_visit_date, $pid);
    $pharmacy_updated = $stmt->execute();
    $stmt->close();

    // Update reception table (only status and next_visit_date)
    ...
    // If status is not PHARMACY, update/insert in department table
    ...
    } elseif ($status_upper === 'NURSING_VITAL' || $status_upper === 'NURSING_CARE') {
    ...
    if ($target_table) {
        ...
        if ($receptionResult && $receptionResult->num_rows > 0) {
            ...
            // fallback to posted values if not found
            $name = '';
            $dept_status = $status;
    ...
    if ($pharmacy_updated && $reception_updated) {
