...
if ($pid) {
    // Always update dental_table
    $stmt = $conn->prepare("UPDATE dental_table SET status=?, notes=?, next_visit_date=? WHERE PID=?");
    $stmt->bind_param("ssss", $status, $notes, $next_visit_date, $pid);
    $dental_updated = $stmt->execute();
    $stmt->close();
    ...
    if ($dental_updated && $reception_updated) {
